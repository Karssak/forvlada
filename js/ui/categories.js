import { apiCall, state, escapeHtml, showToast } from "../core.js";
  ...
  container.innerHTML = categories
    ...
    <div class="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-lg shadow-sm transition-shadow group">
      <div class="flex items-center gap-3">
        <div class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold" style="background-color: ${escapeHtml(cat.color || "#CCCCCC")}">
          ${escapeHtml(cat.name.substring(0, 2).toUpperCase())}
        </div>
        <div>
          <p class="font-bold text-slate-900 text-sm">${escapeHtml(cat.name)}</p>
          <p class="text-xs text-slate-500 capitalize">${escapeHtml(cat.type)}</p>
        </div>
      </div>
